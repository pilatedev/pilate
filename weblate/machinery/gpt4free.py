...
from .forms import GPT4FreeMachineryForm

# Try to import g4f, but handle gracefully if not available
try:
...
class GPT4FreeTranslation(BatchMachineTranslation):
    name = "GPT4Free"
    max_score = 90
    request_timeout = 60

    settings_form = GPT4FreeMachineryForm

    def __init__(self, settings=None) -> None:
        if not G4F_AVAILABLE:
            raise MachineTranslationError("GPT4Free library not available")
    ...
    def _download(
        ...
        *,
        ...
        add_breadcrumb("gpt4free", "prompt", prompt=prompt)
        add_breadcrumb("gpt4free", "chat", content=content)
        ...
        except RetryProviderError as error:
            raise MachineryRateLimitError(str(error)) from error

        translations_string = response.choices[0].message.content
        add_breadcrumb("gpt4free", "response", translations_string=translations_string)
        ...
        # Ignore extra whitespace in response as g4f can be creative in that
    ...
    def get_model(self) -> str:
        ...
        if self._models is None:
            ...
            if models_cache is not None:
                ...
                # Try to get available models from g4f using dynamic listing
                try:
                    from g4f.client import AnyProvider
                    self._models = set(AnyProvider().models)
                except Exception:
                    # Fallback to a default set of models
                    self._models = {
        ...
        if self.settings["model"] == "auto":
            ...
            for model in ["gpt-4o-mini", "gpt-4o", "gpt-4", "llama-3.1-70b"]:
